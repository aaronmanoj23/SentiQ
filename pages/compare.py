...
def render_compare(tickers_dict: dict, year: int, quarter: int, run: bool, uploaded_filings: dict = {}):
    st.markdown(f"## Company Comparison")
    st.markdown(f'<div style="color:#6b6b8a;font-size:0.8rem;letter-spacing:0.15em">Q{quarter} {year} · SIDE-BY-SIDE ANALYSIS</div>', unsafe_allow_html=True)
    ...
    # Run analysis for EDGAR companies
    results = {}
    ...
    if tickers_dict:
        ...
        for i, (company, ticker) in enumerate(tickers_dict.items()):
            ...
            try:
                ...
                st.warning(f"{company} failed: {e}")
        progress.progress(1.0, text="Complete")
        progress.empty()

    # Merge uploaded filings directly (already analyzed)
    for key, filing in uploaded_filings.items():
        display_name = f"{filing['company']} (uploaded)"
    ...
    with col_bar:
        ...
        fig.update_layout(
            **PLOTLY_THEME,
            barmode="group",
            height=300,
            yaxis=dict(tickformat=".0%", gridcolor="#2a2a3a"),
            ...
            **PLOTLY_THEME,
            ...
            polar=dict(
                ...
                radialaxis=dict(
                    ...
                    linecolor="#2a2a3a", tickfont=dict(size=8)
                ),
    ...
    for col, (company, result) in zip(risk_cols, results.items()):
        ...
        with col:
            st.markdown(f'<div style="font-family:\'Syne\',sans-serif;font-weight:700;margin-bottom:0.75rem;color:{color}">{company} ({ticker})</div>', unsafe_allow_html=True)
            
            risks = result["llm_summary"].get("keyRiskSignals", [])[:3]
            for risk in risks:
                ...
                <div style="background:#1a1a24;border-left:3px solid {border};border-radius:4px;padding:0.6rem 0.8rem;margin-bottom:0.4rem;font-size:0.78rem">
                    <div style="color:#e8e8f0;margin-bottom:3px">{risk.get('risk','')}</div>
    ...
    ranked = sorted(results.items(), key=lambda x: x[1]["overall_sentiment"].get("positive", 0), reverse=True)
    ...
    for rank, (company, result) in enumerate(ranked, 1):
        ...
        # Drift indicator (compare pos vs neg)
    ...
    for col, h in zip(header_cols, headers):
        col.markdown(f'<div style="font-size:0.7rem;color:#6b6b8a;text-transform:uppercase;letter-spacing:0.1em;padding-bottom:4px;border-bottom:1px solid #2a2a3a">{h}</div>', unsafe_allow_html=True)
    
    for row in rank_data:
        cols = st.columns([0.5, 2, 1, 1, 1, 1, 1])
        cols[0].markdown(f'<div style="padding:8px 0;font-size:1.1rem">{row["rank"]}</div>', unsafe_allow_html=True)
        cols[1].markdown(f'<div style="padding:8px 0;font-weight:500">{row["company"]} <span style="color:#6b6b8a">({row["ticker"]})</span></div>', unsafe_allow_html=True)
        cols[2].markdown(f'<div style="padding:8px 0;color:{row["color"]};font-weight:500">{row["label"]}</div>', unsafe_allow_html=True)
        cols[3].markdown(f'<div style="padding:8px 0;color:#00d4aa">{row["positive"]:.1%}</div>', unsafe_allow_html=True)
        cols[4].markdown(f'<div style="padding:8px 0;color:#ff4d6d">{row["negative"]:.1%}</div>', unsafe_allow_html=True)
        cols[5].markdown(f'<div style="padding:8px 0">{row["confidence"]:.1%}</div>', unsafe_allow_html=True)
        cols[6].markdown(f'<div style="padding:8px 0;color:{row["spread_color"]}">{row["spread_sign"]}{row["spread"]:.1%}</div>', unsafe_allow_html=True)


def _get_domain(ticker: str) -> str:
    domains = {
        "AAPL": "apple.com", "MSFT": "microsoft.com", "GOOGL": "google.com",
        "AMZN": "amazon.com", "META": "meta.com", "GS": "goldmansachs.com",
        "JPM": "jpmorganchase.com", "MS": "morganstanley.com",
